...
static AstFuncDefNodePtr FunctionDefinition(void) {
  ...
  while (curToken.kind == TK_ID) {
    ...
    if (para_cnt < NUM_OF_ARGS_IN_REGS) {
      func->paras[para_cnt].offset = -((para_cnt + 1) * CPU_WORD_SIZE_IN_BYTES);
    } else {
      func->paras[para_cnt].offset = OFFSET_OF_PARAMETER_7 + (para_cnt - NUM_OF_ARGS_IN_REGS) * CPU_WORD_SIZE_IN_BYTES;
    }
    para_cnt++;
    if (para_cnt > MAX_PARAMETER_CNT) {
      Error(__FILE__,
            __LINE__,
            "only %d parameters allowed",             
            MAX_PARAMETER_CNT);
    }
    NEXT_TOKEN;
...
void EmitFuncDefNode(AstFuncDefNodePtr func) {
  ...
  while (func) {
    ...
    **********************************/
     ...
     ****************************************************************************************/
    EmitComments("# Simple policy for saving arguments passed in registers");
    for (int i = 0; (i < func->para_cnt) && (i < NUM_OF_ARGS_IN_REGS); i++) {
      EmitAssembly("movq %%%s, %d(%%rbp)", x64ArgRegs[i], -(i + 1) * CPU_WORD_SIZE_IN_BYTES);
    }
    EmitComments("\n");
