...
static int defaultNextChar(void) { return EOF; }
...
// Get a token (like an English word, but a token consists of a kind and a
// value.)
Token GetToken(void) {
    Token token;
    ...
    if (curChar == EOF) { // end of file
        ...
        if (token.kind != TK_NA) {
            ...
            Error("Out of our alphabet: char '%c' or \'0x%x\'", curChar,
                  curChar);
        }
    }
    return token;
