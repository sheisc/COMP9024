...
static int DoGetPathInDfs(struct Graph *pGraph, 
                            long u, 
                            long source, 
                            long sink, 
                            long *parents, 
    ...
    for (int v = 0; v < pGraph->n; v++) {
        if (!visited[v] && MatrixElement(pGraph, u, v) > 0) {
            parents[v] = u;
            if (DoGetPathInDfs(pGraph, v, source, sink, parents, visited)) {
                return 1;
            }
        }
    }
...
/*
    GetPathInDfs() returns 1 when it finds a path, otherwise it returns 0.
 */
static int GetPathInDfs(struct Graph *pGraph, 
                        long source, 
                        long sink, 
                        long *parents,
                        int *visited) {
    //memset(visited, 0, pGraph->n * sizeof(int));
    for (long i = 0; i < pGraph->n; i++) {
        visited[i] = 0;
    }
    int found =  DoGetPathInDfs(pGraph, source, source, sink, parents, visited);
    return found;
}
...
AdjMatrixElementTy FordFulkersonMF(struct Graph *pGraph, long source, long sink) {
    ...
    GenOneImage(pGraph, "FordFulkersonMF", "images/FordFulkersonMF", imgCount, visited, path, 0, 0);
    imgCount++;

    while (GetPathInDfs(pGraph, source, sink, parents, visited)) {
        ...
        GenOneImage(pGraph, "FordFulkersonMF", "images/FordFulkersonMF", imgCount, visited, path, len, 1);
        imgCount++;
        ...
        GenOneImage(pGraph, "FordFulkersonMF", "images/FordFulkersonMF", imgCount, visited, path, 0, 1);
        imgCount++;
